...
// ============================================================================
// Kernel 8b: Batched causal SDPA, tiled online-softmax (Phase 3, batched prefill)
// ============================================================================
//
// Computes scaled dot-product attention for N queries against a kv_len-position
// cache with per-query causal mask:
//
//     out[q, h, d] = sum_{p < kv_max(q)} softmax(q*K[p,kv_h]/sqrt(d)) * V[p,kv_h,d]
//
// where kv_max(q) = start_pos + q + 1.
//
// Flash-Attention-style online softmax across `kv_len` positions processed in
// chunks of `BATCHED_SDPA_TILE_SIZE`. Two kernels:
//
//   1. `attn_sdpa_causal_tile_accumulate` — process one tile, update running
//      (max, denom, v_partial) per (query, head).
//   2. `attn_sdpa_causal_tile_finalize` — divide v_partial by running_denom
//      to produce the final output.
//
// Running-state buffers: max/denom [N, H], v_partial [N, H, head_dim]. Encoder
// pre-initializes max to -INF and denom to 0 (v_partial untouched until first
// non-masked tile writes it).
//
// Grid per accumulate dispatch: N * num_heads threadgroups, 128 threads each.
// Causal cutoff is per-query inside the kernel — a TG whose entire tile is
// past kv_max returns without touching running state.
//
// Bit-exact-against-single-shot for kv_len ≤ BATCHED_SDPA_TILE_SIZE and N=1;
// multi-tile and N>1 paths cosine ≥ 0.9999 vs tokenwise CPU oracle.

constant uint BATCHED_SDPA_TILE_SIZE = 4096;
constant uint BATCHED_SDPA_THREADS   = 128;

kernel void attn_sdpa_causal_init_running(
    device float* running_max   [[buffer(0)]],   // [N, H]
    device float* running_denom [[buffer(1)]],   // [N, H]
    constant uint& total        [[buffer(2)]],   // N * H
    uint tid [[thread_position_in_grid]]
) {
    if (tid >= total) return;
...
kernel void attn_sdpa_causal_tile_accumulate(
    device const float* Q              [[buffer(0)]],  // [N, H, head_dim]
    device const float* K_cache        [[buffer(1)]],  // [kv_len, kv_dim]
    device const float* V_cache        [[buffer(2)]],  // [kv_len, kv_dim]
    device float*       running_max    [[buffer(3)]],  // [N, H]
    device float*       running_denom  [[buffer(4)]],  // [N, H]
    device float*       v_partial      [[buffer(5)]],  // [N, H, head_dim]
    ...
    constant uint&      tile_start     [[buffer(12)]],
    constant uint&      tile_end       [[buffer(13)]],
    constant float&     softmax_scale  [[buffer(14)]],
    constant uint&      is_first_tile  [[buffer(15)]],
    uint tg_idx [[threadgroup_position_in_grid]],      // linearized: q * H + h
    uint lid    [[thread_position_in_threadgroup]]
) {
    uint q_idx = tg_idx / num_heads;
    uint h     = tg_idx % num_heads;
    if (q_idx >= n_tokens) return;
    uint tile_size = tile_end - tile_start;
    if (tile_size == 0) return;

    uint kv_h      = h / heads_per_kv;
    uint query_pos = start_pos + q_idx;
    uint kv_max    = query_pos + 1;  // exclusive — positions ≥ kv_max are masked

    threadgroup float scores[BATCHED_SDPA_TILE_SIZE];
    threadgroup float simd_scratch[8];

    device const float* q_h =
        Q + ((size_t)q_idx * num_heads + h) * head_dim;

    // 1. scores[i] = (q · K[tile_start+i, kv_h]) * scale, with causal mask.
    for (uint i = lid; i < tile_size; i += BATCHED_SDPA_THREADS) {
        uint t = tile_start + i;
        float s;
        if (t >= kv_max) {
            s = -INFINITY;
        } else {
            device const float* k_t =
                K_cache + (size_t)t * kv_dim + (size_t)kv_h * head_dim;
            float acc = 0.0f;
            for (uint d = 0; d < head_dim; ++d) {
                acc = fma(q_h[d], k_t[d], acc);
            }
            s = acc * softmax_scale;
        }
        scores[i] = s;
    ...
    uint num_simdgroups = BATCHED_SDPA_THREADS / 32;
    float local_max = -INFINITY;
    for (uint i = lid; i < tile_size; i += BATCHED_SDPA_THREADS) {
        local_max = max(local_max, scores[i]);
    }
    float smax = simd_max(local_max);
    if (simd_lane == 0) { simd_scratch[simd_group] = smax; }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    if (simd_group == 0) {
    ...
    threadgroup_barrier(mem_flags::mem_threadgroup);
    float tile_max = simd_scratch[0];

    // If the entire tile is masked for this query, leave running state alone.
    if (tile_max == -INFINITY) return;
    ...
    float ssum = simd_sum(local_sum);
    if (simd_lane == 0) { simd_scratch[simd_group] = ssum; }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    if (simd_group == 0) {
        float v = (simd_lane < num_simdgroups)
            ? simd_scratch[simd_lane]
    ...
    threadgroup_barrier(mem_flags::mem_threadgroup);
    float tile_sum = simd_scratch[1];

    // 4. Merge with running state.
    //    prev_max == -INF on first non-masked tile for this (q, h) — encoder
    //    pre-initialized running_max to -INF. Reading running_max[h_idx]
    //    works whether it's the actual first tile or a later one where prior
    //    tiles were all masked.
    uint state_idx = q_idx * num_heads + h;
    float prev_max   = (is_first_tile != 0u) ? -INFINITY : running_max[state_idx];
    float prev_denom = (is_first_tile != 0u) ? 0.0f      : running_denom[state_idx];
    ...
    // 5. v_partial[q_idx, h, d] update across head_dim.
    device float* v_part_h =
        v_partial + ((size_t)q_idx * num_heads + h) * head_dim;
    for (uint d = lid; d < head_dim; d += BATCHED_SDPA_THREADS) {
        float tile_partial = 0.0f;
        for (uint i = 0; i < tile_size; ++i) {
            uint t = tile_start + i;
            // scores[i] is already exp(score - tile_max); masked positions
            // had score=-INF → exp(-INF - tile_max) = 0 (when tile_max finite).
            tile_partial = fma(
                scores[i],
                V_cache[(size_t)t * kv_dim + (size_t)kv_h * head_dim + d],
                tile_partial);
        }
        float prev_v = (is_first_tile != 0u) ? 0.0f : v_part_h[d];
        v_part_h[d] = prev_v * scale_old + tile_partial * scale_new;
    }

    if (lid == 0) {
        running_max[state_idx]   = new_max;
        running_denom[state_idx] = new_denom;
    }
}

kernel void attn_sdpa_causal_tile_finalize(
    device const float* v_partial     [[buffer(0)]],  // [N, H, head_dim]
    device const float* running_denom [[buffer(1)]],  // [N, H]
    device float*       out           [[buffer(2)]],  // [N, H, head_dim]
    ...
    if (tid >= total) return;
    uint d_within = tid % head_dim;
    uint qh = tid / head_dim;             // qh = q_idx * num_heads + h
    (void)d_within;
    float denom = running_denom[qh];
    float inv = (denom > 0.0f) ? (1.0f / denom) : 0.0f;
    out[tid] = v_partial[tid] * inv;
}


// ============================================================================
// FlashAttention-2 causal SDPA — batched prefill (replaces the 3-kernel
// attn_sdpa_causal_* path above).
// ============================================================================
//
