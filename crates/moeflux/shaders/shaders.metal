...
// ============================================================================
// Kernel 1e: tiled 4-bit dequant matmul (simdgroup_matrix)
// ============================================================================
//
// Real tiled matmul for batched prefill. The v3 / fast matvec kernels above
// own one (token, output-row) pair per threadgroup and re-stream the weight
// matrix once per token — for an 8192-token prefill that reads the 4-bit
// weights 8192 times, leaving the kernel memory-bound at ~5% of GPU peak.
//
// This kernel tiles the token axis: one threadgroup computes a BM x BN output
// tile, so each weight slab is read once per (n_tokens / BM) tiles and reused
// across BM tokens. Arithmetic intensity rises until the kernel is
// compute-bound, and simdgroup_float8x8 matrices feed the ALUs.
//
//   out[token][row] = sum_k W[row][k] * x[token][k]
//   C[M,N] = A[M,K] @ B[K,N]   M = n_tokens, N = out_dim, K = in_dim
//   A[m][k] = x[m][k]          B[k][n] = dequant(W[n][k])
//
// K is walked in BK slabs; each slab stages A (activations) and a dequantized
// B (4-bit weights) into threadgroup memory once. TG = 128 threads = 4
// simdgroups in a 2x2 layout; each simdgroup owns a 32x32 output quadrant
// = 4x4 of 8x8 accumulators.
//
// The staged tiles are half (f16): half the threadgroup memory of f32
// tiles, so ~3 threadgroups fit per GPU core instead of 1 — enough
// occupancy to hide staging latency. f16 is hardware-native on the M2
// simdgroup matrix unit (bf16 simdgroup matmul is emulated there and
// measured ~3x slower). The matmul accumulates into f32 simdgroup
// matrices, so accuracy holds (diff oracle: cosine > 0.9999); the staged
// activations / dequantized weights are well within f16 range.
//
// Grid: tg_flat linearizes (m_tile, n_tile) as m_tile + n_tile * num_m_tiles;
// dispatch ceil(n_tokens/BM) * ceil(out_dim/BN) threadgroups of 128 threads.

#define MM_BM 64
#define MM_BN 64
#define MM_BK 32
#define MM_SG_COUNT 4

kernel void mul_mm_4bit(
    device const uint32_t* W_packed   [[buffer(0)]],  // [out_dim, in_dim/8]
    device const uint16_t* scales     [[buffer(1)]],  // [out_dim, num_groups] bf16
    device const uint16_t* biases     [[buffer(2)]],  // [out_dim, num_groups] bf16
    device const float*    x          [[buffer(3)]],  // [n_tokens, in_dim]
    device float*          out        [[buffer(4)]],  // [n_tokens, out_dim]
    constant uint&         out_dim    [[buffer(5)]],
    constant uint&         in_dim     [[buffer(6)]],
    constant uint&         group_size [[buffer(7)]],
    constant uint&         n_tokens   [[buffer(8)]],
    constant uint&         num_m_tiles [[buffer(9)]],  // ceil(n_tokens / MM_BM)
    uint tg_flat [[threadgroup_position_in_grid]],
    uint lid     [[thread_position_in_threadgroup]],
    uint sg_id   [[simdgroup_index_in_threadgroup]]
) {
    uint m_tile = tg_flat % num_m_tiles;
    uint n_tile = tg_flat / num_m_tiles;
    uint M0 = m_tile * MM_BM;
    uint N0 = n_tile * MM_BN;

    uint packed_cols = in_dim / 8;
    uint num_groups  = in_dim / group_size;

    threadgroup half As[MM_BM * MM_BK];   // 64*32*2 = 4 KB
    threadgroup half Bs[MM_BK * MM_BN];   // 32*64*2 = 4 KB

    // This simdgroup's 32x32 output quadrant.
    uint qm = sg_id / 2;   // token quadrant (0,1)
    uint qn = sg_id % 2;   // row   quadrant (0,1)
    ...
    for (uint k0 = 0; k0 < in_dim; k0 += MM_BK) {
        // Stage A: As[m][k] = x[(M0+m)][k0+k], zero-padded out of range.
        for (uint idx = lid; idx < MM_BM * MM_BK; idx += 128) {
            uint m = idx / MM_BK;
            uint k = idx % MM_BK;
            uint gm = M0 + m;
            uint gk = k0 + k;
            As[idx] = (gm < n_tokens && gk < in_dim)
                ? half(x[(size_t)gm * in_dim + gk])
                : half(0.0f);
        }
        // Stage B: Bs[k][n] = dequant(W[N0+n][k0+k]), zero-padded.
        for (uint idx = lid; idx < MM_BK * MM_BN; idx += 128) {
            uint k = idx / MM_BN;
            uint n = idx % MM_BN;
            uint gn = N0 + n;
            uint gk = k0 + k;
            ...
            if (gn < out_dim && gk < in_dim) {
                uint word = W_packed[(size_t)gn * packed_cols + gk / 8];
                uint nib  = (word >> ((gk % 8) * 4)) & 0xF;
                uint g    = gk / group_size;
                float scale = bf16_to_f32(scales[(size_t)gn * num_groups + g]);
                float bias  = bf16_to_f32(biases[(size_t)gn * num_groups + g]);
                w = float(nib) * scale + bias;
            }
            Bs[idx] = half(w);
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);

        // Matmul this slab: 4 sub-tiles of 8 along K.
        for (uint kk = 0; kk < MM_BK; kk += 8) {
            simdgroup_half8x8 a_frag[4];
            simdgroup_half8x8 b_frag[4];
            for (uint mi = 0; mi < 4; mi++) {
                uint m = qm * 32 + mi * 8;
                simdgroup_load(a_frag[mi], &As[m * MM_BK + kk], MM_BK);
            }
            for (uint ni = 0; ni < 4; ni++) {
                uint n = qn * 32 + ni * 8;
                simdgroup_load(b_frag[ni], &Bs[kk * MM_BN + n], MM_BN);
            }
            for (uint mi = 0; mi < 4; mi++)
                for (uint ni = 0; ni < 4; ni++)
                    simdgroup_multiply_accumulate(
                        acc[mi][ni], a_frag[mi], b_frag[ni], acc[mi][ni]);
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }

    // Store. Fast path for fully in-bounds 8x8 tiles; edge path guards the
    // ragged tiles at the n_tokens / out_dim boundaries.
    threadgroup float edge[MM_SG_COUNT * 64];
    for (uint mi = 0; mi < 4; mi++) {
        for (uint ni = 0; ni < 4; ni++) {
            uint gm = M0 + qm * 32 + mi * 8;
            uint gn = N0 + qn * 32 + ni * 8;
            if (gm + 8 <= n_tokens && gn + 8 <= out_dim) {
                simdgroup_store(acc[mi][ni],
                                &out[(size_t)gm * out_dim + gn], out_dim);
            } else if (gm < n_tokens && gn < out_dim) {
                threadgroup float* e = &edge[sg_id * 64];
                simdgroup_store(acc[mi][ni], e, 8);
                simdgroup_barrier(mem_flags::mem_threadgroup);
                for (uint t = lid % 32; t < 64; t += 32) {
                    uint r = t / 8;
                    uint c = t % 8;
                    if (gm + r < n_tokens && gn + c < out_dim) {
                        out[(size_t)(gm + r) * out_dim + (gn + c)] = e[t];
                    }
                }
            }
        }
